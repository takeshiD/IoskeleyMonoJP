...
COPYRIGHT = """[Iosevka]
Copyright (c) 2015-2024 Belleve Invis (be5invis) https://github.com/be5invis/Iosevka

[Ioskeley Mono]
Copyright (c) 2025, Ahmed Hatem (ahatem) https://github.com/ahatem/IoskeleyMono
...
[Ioskeley Mono JP]
Copyright 2025 Ioskeley Mono JP Project Authors
"""  # noqa: E501
...
def generate_font(jp_style, eng_style, merged_style):
    ...
    # Ioskeley Mono の 0 は元々ドットゼロ (Berkeley Mono 風) なので、
    # UDEV Gothic のようなスラッシュゼロ化は行わず、そのまま維持する。
...
def add_nerd_font_glyphs(jp_font, eng_font):
    """Nerd Fontのグリフを追加する"""
    # Nerd Fontのソースはスタイル毎に開き直す。
    # 新しめの FontForge では mergeFonts() がマージ元フォントを閉じてしまうため、
    # グローバルにキャッシュした nerd_font を次スタイルで再利用できない。
    # さらに fontforge.open() は開いたフォントをパスでキャッシュするため、同じパスを
    # 開き直すと「閉じられた無効なハンドル」が返る。これを避けるため、毎回ユニークな
    # 一時ファイルへコピーしてから開く。
    tmp_nerd_path = f"{BUILD_FONTS_DIR}/_nerd_{uuid.uuid4().hex}.ttf"
    shutil.copy(f"{SOURCE_FONTS_DIR}/SymbolsNerdFont-Regular.ttf", tmp_nerd_path)
    nerd_font = fontforge.open(tmp_nerd_path)
    ...
    # 一時ファイルを削除 (mergeFonts 済みなので不要)
    try:
        os.remove(tmp_nerd_path)
    except OSError:
        pass
    jp_font.selection.none()
    eng_font.selection.none()
